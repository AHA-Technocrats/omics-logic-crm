{!! view_render_event('admin.campaigns.view.inventory.before', ['product' => $product]) !!}

<!-- Product Inventories Component -->
<v-product-inventories></v-product-inventories>

{!! view_render_event('admin.campaigns.view.inventory.after', ['product' => $product]) !!}
    ...
    >
        <div class="p-4">
            <div class="flex flex-col gap-4">
                {!! view_render_event('admin.campaigns.view.inventory.table.before', ['product' => $product]) !!}
                    ...
                    <x-admin::table>
                        ...
                        <x-admin::table.thead>
                            <x-admin::table.thead.tr>
                                <x-admin::table.th>
                                    @lang('admin::app.campaigns.view.inventory.warehouse')
                                </x-admin::table.th>

                                <x-admin::table.th >
                                    @lang('admin::app.campaigns.view.inventory.in-stock')
                                </x-admin::table.th>

                                <x-admin::table.th>
                                    @lang('admin::app.campaigns.view.inventory.allocated')
                                </x-admin::table.th>

                                <x-admin::table.th>
                                    @lang('admin::app.campaigns.view.inventory.on-hand')
                                </x-admin::table.th>

                                <x-admin::table.th>
                                    @lang('admin::app.campaigns.view.inventory.actions')
                                </x-admin::table.th>
                            </x-admin::table.thead.tr>
                        </x-admin::table.thead>
                            ...
                            <template v-for="warehouse in productWarehouses">
                                <x-admin::table.tbody.tr class="hover:bg-gray-50 dark:hover:bg-gray-950">
                                    ...
                                    <x-admin::table.td>
                                        ...
                                        >
                                            @lang('admin::app.campaigns.view.inventory.assign')
                                        </div>
                                    </x-admin::table.td>
                ...
                {!! view_render_event('admin.campaigns.view.inventory.table.after', ['product' => $product]) !!}

                {!! view_render_event('admin.campaigns.view.inventory.source.before', ['product' => $product]) !!}
                    ...
                    >
                        <x-slot:toggle>
                            ...
                            >
                                ...
                                @lang('admin::app.campaigns.view.inventory.add-warehouse')
                            </button>
                        </x-slot:toggle>
                        <x-slot:menu class="max-h-[200px] overflow-auto">
                            {!! view_render_event('admin.campaigns.view.inventory.source.menu.item.before', ['product' => $product]) !!}
                            ...
                            {!! view_render_event('admin.campaigns.view.inventory.source.menu.item.after', ['product' => $product]) !!}
                        </x-slot>
                    </x-admin::dropdown>
                </div>

                {!! view_render_event('admin.campaigns.view.inventory.source.after', ['product' => $product]) !!}
            </div>

            {!! view_render_event('admin.campaigns.view.inventory.form_controls.before', ['product' => $product]) !!}
            ...
            >
                <form
                    ...
                    ref="locationForm"
                >
                    {!! view_render_event('admin.campaigns.view.inventory.form_controls.drawer.before', ['product' => $product]) !!}
                    ...
                    >
                        <!-- Drawer Header -->
                        <x-slot:header>
                            {!! view_render_event('admin.campaigns.view.inventory.form_controls.drawer.header.before', ['product' => $product]) !!}
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                >
                                     @lang('admin::app.campaigns.view.inventory.save')
                                </button>
                            </div>

                            {!! view_render_event('admin.campaigns.view.inventory.form_controls.drawer.header.after', ['product' => $product]) !!}
                        </x-slot>

                        <!-- Drawer Content -->
                        <x-slot:content>
                            {!! view_render_event('admin.campaigns.view.inventory.form_controls.drawer.content.before', ['product' => $product]) !!}

                            <v-warehouse-location-inventories
                                :warehouse="selectedWarehouse"
                            ></v-warehouse-location-inventories>

                            {!! view_render_event('admin.campaigns.view.inventory.form_controls.drawer.content.after', ['product' => $product]) !!}
                        </x-slot>
                    </x-admin::drawer>

                    {!! view_render_event('admin.campaigns.view.inventory.form_controls.drawer.after', ['product' => $product]) !!}
                </form>
            </x-admin::form>

            {!! view_render_event('admin.campaigns.view.inventory.form_controls.after', ['product' => $product]) !!}
        </div>
    </script>
    ...
    >
        <div class="flex flex-col gap-2">
                    ...
                    <x-admin::table.thead>
                        <x-admin::table.thead.tr>
                            <x-admin::table.th class="!w-56">
                                @lang('admin::app.campaigns.view.inventory.location')
                            </x-admin::table.th>
            ...
            >
                ...
                @lang('admin::app.campaigns.view.inventory.add-more')
            </button>
        </div>
    </script>
        ...
        <x-admin::table.tbody.tr>
            <x-admin::table.td class="!px-2">
                <x-admin::form.control-group>
                    <x-admin::lookup
                        ...
                        v-model="location['id']"
                        rules="required"
                        :placeholder="trans('admin::app.campaigns.view.inventory.location')"
                        :label="trans('admin::app.campaigns.view.inventory.location')"
                        @on-selected="add"
            ...
            <x-admin::table.td class="!px-2">
                <x-admin::form.control-group.control
                    ...
                    :label="trans('admin::app.campaigns.view.inventory.in-stock')"
                    :placeholder="trans('admin::app.campaigns.view.inventory.in-stock')"
                />
            ...
            <x-admin::table.td class="!px-2">
                <x-admin::form.control-group.control
                    ...
                    :label="trans('admin::app.campaigns.view.inventory.allocated')"
                    :placeholder="trans('admin::app.campaigns.view.inventory.allocated')"
                />
        ...
        app.component('v-product-inventories', {
            ...
            methods: {
                ...
                getProductWarehouses() {
                    this.$axios.get("{{ route('admin.campaigns.warehouses', $product->id) }}")
                        .then(response => {
                ...
                onSubmit(params, { setErrors }) {
                    ...
                    this.$axios.post("{{ route('admin.campaigns.inventories.store', ['id' => $product->id, 'warehouseId' => 'warehouseId']) }}".replace('warehouseId', this.selectedWarehouse.id), formData, {
                        headers: {
