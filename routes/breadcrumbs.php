...
Breadcrumbs::for('settings.campaign_categories', function (BreadcrumbTrail $trail) {
    $trail->parent('settings');
    $trail->push(trans('admin::app.layouts.campaign-categories'), route('admin.settings.campaign_categories.index'));
});
...
// Configuration > Section (e.g. Lead Score / Email domains)
Breadcrumbs::for('configuration.edit', function (BreadcrumbTrail $trail) {
    ...
    $section = system_config()->getItems()->firstWhere('key', $slug);

    if (! $section) {
        return;
    }

    $trail->push($section->getName(), route('admin.configuration.index'));

    $child = $section->getChildren()[$slug2] ?? null;

    if ($child) {
        $trail->push(
            $child->getName(),
            route('admin.configuration.index', ['slug' => $slug, 'slug2' => $slug2])
        );
    }
});
